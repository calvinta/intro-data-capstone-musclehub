...
LEFT JOIN purchases
       ...
       AND purchases.email = purchases.email
...
# The difference between the two test groups is significant.
...
just_apps = df.loc[df['is_application'] == 'Application'].reset_index()
just_apps.head()
...
contingency = [[202, 2302],
               [251, 2249]]
chi2, pval, dof, expected = chi2_contingency(contingency)
pval


# There is a significant difference in memberships between Group A and Group B across all people who visit MuscleHub.
...
plt.savefig('percent_of_applicants_purchase_membership.png')
plt.show()
